...
def draggable_block():
    return f"""
        <div id="draggable-block" draggable="true" ondragstart="drag(event)">
            Block
        </div>
        <script>
            function drag(event) {{
...
def main():
    ...
    # Display drop area
    drop_area = st.markdown("<div id='code-area' ondrop='drop(event)' ondragover='allowDrop(event)'></div>", unsafe_allow_html=True)
